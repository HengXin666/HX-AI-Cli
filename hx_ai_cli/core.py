...
MESSAGES: dict[str, dict[str, str]] = {
    "en": {
        ...
        "main_clear": "Clear saved tasks for this workspace",
        "main_clear_exit": "Clear workspace tasks and exit",
        "main_exit": "Exit",
        ...
        "cleared_tasks": "Cleared {count} task(s).",
        ...
        "worktree_root": "Worktree storage directory",
    ...
    "zh": {
        ...
        "main_clear": "清空当前工作区任务",
        "main_clear_exit": "清空当前工作区任务并退出",
        "main_exit": "退出",
        ...
        "worktrees_path": "Worktree 目录：{path}",
        "config_prompt": "配置",
...
def entry_script_path() -> Path:
    script = Path(__file__).resolve().parent.parent / "aiw.py"
    return script if script.exists() else Path(__file__).resolve()
...
def worktrees_base_dir() -> Path:
    return config_dir() / DEFAULT_TASKS_DIR
...
def merge_defaults(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if worktree.get("root") == ".ai-worktrees":
        worktree["root"] = str(worktrees_base_dir())
    return merged
...
def create_worktree(repo: Path, task_name: str, config: dict[str, Any]) -> tuple[Path, str]:
    ...
    configured_root = Path(config["worktree"].get("root") or str(worktrees_base_dir())).expanduser()
    base = configured_root if configured_root.is_absolute() else worktrees_base_dir() / configured_root
    repo_slug = slugify(str(repo.resolve()))
    base = base / repo_slug
...
def same_workspace(record: dict[str, Any], workspace: Path) -> bool:
    try:
        return Path(str(record.get("workspace", ""))).expanduser().resolve() == workspace.resolve()
    except OSError:
        return False
...
def remove_worktree_for_record(config: dict[str, Any], record: dict[str, Any], confirm: bool = True) -> None:
    ...
    if confirm and not ask_confirm(tr(config, "cleanup_worktree_confirm", cwd=cwd, branch=branch or "-"), default=True):
        return
...
def clear_workspace_tasks(config: dict[str, Any], workspace: Path) -> None:
    sessions = workspace_sessions(workspace)
    if not sessions:
        print_error(tr(config, "no_sessions"))
        return
    if not ask_confirm(tr(config, "clear_tasks_confirm"), default=False):
        return
    ...
    save_sessions([item for item in load_sessions() if item.get("id") not in ids])
...
def start_existing(config: dict[str, Any], workspace: Path | None = None) -> int | None:
    record = choose_existing_session(config, workspace)
    ...
    while True:
        ...
        mode = ask_select(
            ...
            [
                ...
                questionary.Choice(title=tr(config, "main_clear"), value="clear"),
                questionary.Choice(title=tr(config, "main_clear_exit"), value="clear_exit"),
                questionary.Choice(title=tr(config, "main_exit"), value="exit"),
            ],
        )
        if mode == "continue":
            result = start_existing(config, workspace)
            if result is None:
                continue
            return result
        if mode == "new":
        ...
        if mode == "vscode":
            ...
            continue
        if mode == "clear":
            clear_workspace_tasks(config, workspace)
            continue
        if mode == "clear_exit":
            clear_workspace_tasks(config, workspace)
            return 0
...
def show_config(config: dict[str, Any]) -> None:
    ...
    print_line(tr(config, "worktrees_path", path=worktrees_base_dir()))
